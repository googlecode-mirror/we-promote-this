...
error_reporting ( E_ALL ^ E_NOTICE ^ E_WARNING ); // Errors only
...
define ( "mysqlServerIP", DB_HOST );
define ( "mysqlServerIP2", DB_HOST );
...
class WPTRedirect {
	...
	function __construct() {
		ob_clean ();
		$userLink = $_REQUEST ["uid"];
		$mainUser = $this->getUser ( $userLink );
		if (isset ( $mainUser )) {
	...
	function returnUserAffiliateLink(User $user) {
		...
		if (isset ( $user )) {
			...
			if ($user->getPass ()) {
				$user->setPass ( false );
				$newUser = $this->getUser ( $user->upline );
...
class User {
	...
	public function User($i, $li = "cq2smooth", $u = "cq2smooth", $l = 0, $e, $p = false) {
		$this->id = $i;
		$this->link = $li;
		$this->upline = $u;
		$this->level = $l;
