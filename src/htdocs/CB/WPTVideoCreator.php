...
class WPTVideoCreator extends CBAbstract {
	...
	function createVideo($pid, $title, $description, $link, $mp3) {
		...
		$format = "mp4";
		$outputFolder = $this -> root . "/Video/CreatedVideos/" . "$pid/";
		...
		if ($return_value == 0) {
			$status = $this -> uploadFile($outputFile, $pid);
	...
	function uploadFile($file, $pid) {
		...
		$encrypted_file_array = $this -> encrypt($this -> json_encode_fake(array("file" => urlencode(basename($file)), "pid" => $pid)));
