			...
			foreach ($argv as $value) {
				...
				switch ($key) {
					case "update-undone" :
						...
						WHERE k.id IS NULL AND p.id NOT IN (
						SELECT id
						FROM BadIDs
						WHERE COUNT>=3)
						ORDER BY 
						k.updated_at ASC, RAND()
						LIMIT " . $keywordLimit;
					...
					case "update-random" :
						$query = "SELECT p.id
						FROM products AS p
						INNER JOIN keywords AS k ON k.id=p.id
						WHERE p.id NOT IN (
						SELECT id
						FROM BadIDs
						WHERE COUNT>=3)
					...
					case "options" :
						$options = array(
							...
							"update-undone" => "Update the ProductUpdateDBQuery.txt file with info for ID found in the products table but not in the keywords table",
							"update" => "Update the ProductUpdateDBQuery.txt file with update queries for all id's found in the products table",
							"update-random"=>"Update the ProductUpdateDBQuery.txt file with update queries for ".$keywordLimit." random id's found in the products ad keywords table ordered by updated_at ascending",
							"update-query" => "Update the ProductUpdateDBQuery.txt file with update queries using the passed query",
							"runupdatequery" => "Run all update queries from the ProductUpdateDBQuery.txt file",
							"id" => "Update the keywords for id into the ProductUpdateDBQuery.txt file"
						);
						$space = 0;
