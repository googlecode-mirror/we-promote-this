...
require_once 'Account/GmailPhoneVerifier.php';
class WPTCreateYoutubeAccountForUsers extends CBAbstract {
	...
	function constructClass() {
		...
		if (isset ( $_REQUEST ['phone_verification'] ) ) {
			// Search gmail for phone verification number and return it as json

			$gpv = new GmailPhoneVerifier('frostbyte07@gmail.com', 'Cingularneeuq01');
			$verificationNumber = $gpv->getPhoneVerificationNumber();
			
			$json = json_encode ( array ("verification_number" => $verificationNumber ) );
			echo $json;
		} else {
			
			// Open the html and body tags
			echo ('
		<html>
		<body>
		<head>
		...
		AND u4.meta_key='clickbank_clerk_api_key' AND u4.meta_value IS NOT NULL";
			...
			echo ('
		<form id="wptyoutubechannelcreation" name="wptyoutubechannelcreation" action="WPTCreateYoutubeAccountForUsers.php" method="POST">
		...
		');
			...
			// Create form for verification phone number and input to get the code from email
			echo ('
		<form  id="wptgoogleverification" name="wptgoogleverification"> 
		<input type="text" id="phone_number" name="phone_number" value="4432288112"><br>
		<input type="text" id="verification_number" name="verification_number" value=""><br>
		<input type="button" name="get_phone_verification"  value="Get Verification Code"
		onClick="
				$(\'#verification_number\')[0].value = \'\';
				$.ajax({
			    url: \'WPTCreateYoutubeAccountForUsers.php\',
			    data: \'phone_verification=1\',
			    dataType: \'text json\',
			    success: function(response){
			      $(\'#verification_number\')[0].value = response[\'verification_number\'];
			    }
			});
		"
		>
