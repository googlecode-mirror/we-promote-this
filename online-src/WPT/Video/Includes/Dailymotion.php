...
class Dailymotion {
	...
	protected $grantType = null, $grantInfo = null, $session = null, $storeSession = true;
	/**
	...
	public function getAuthorizationUrl($redirectUri = null, $scope = array(), $display = 'page') {
		...
		return $this->oauthAuthorizeEndpointUrl . '?' . http_build_query ( array ('response_type' => 'code', 'client_id' => $this->grantInfo ['key'], 'redirect_uri' => $this->grantInfo ['redirect_uri'], 'scope' => is_array ( $scope ) ? implode ( ' ', $scope ) : $scope, 'display' => $display ), null, '&' );
	}
	/**
	...
	public function call($method, $args = array()) {
		...
		$payload = json_encode ( array ('call' => $method, 'args' => $args ) );
		$status_code = null;
		try {
	...
	protected function getAccessToken($forceRefresh = false) {
		...
		if (isset ( $session ) && isset ( $session ['grant_type'] ) && $session ['grant_type'] === $this->grantType) {
			...
			if (isset ( $session ['refresh_token'] )) {
				...
				$session = $this->oauthTokenRequest ( array ('grant_type' => 'refresh_token', 'client_id' => $this->grantInfo ['key'], 'client_secret' => $this->grantInfo ['secret'], 'scope' => isset ( $this->grantInfo ['scope'] ) ? $this->grantInfo ['scope'] : null, 'refresh_token' => $session ['refresh_token'] ) );
		...
		try {
			if ($this->grantType === self::GRANT_TYPE_AUTHORIZATION) {
				if (isset ( $_GET ['code'] )) {
					...
					$session = $this->oauthTokenRequest ( array ('grant_type' => 'authorization_code', 'client_id' => $this->grantInfo ['key'], 'client_secret' => $this->grantInfo ['secret'], 'scope' => isset ( $this->grantInfo ['scope'] ) ? $this->grantInfo ['scope'] : null, 'code' => $_GET ['code'], 'redirect_uri' => $this->grantInfo ['redirect_uri'] ) );
				...
				$session = $this->oauthTokenRequest ( array ('grant_type' => 'client_credentials', 'client_id' => $this->grantInfo ['key'], 'client_secret' => $this->grantInfo ['secret'], 'scope' => isset ( $this->grantInfo ['scope'] ) ? $this->grantInfo ['scope'] : null ) );
				$session ['grant_type'] = $this->grantType;
				$this->setSession ( $session );
				return $session ['access_token'];
				...
				$session = $this->oauthTokenRequest ( array ('grant_type' => 'password', 'client_id' => $this->grantInfo ['key'], 'client_secret' => $this->grantInfo ['secret'], 'scope' => isset ( $this->grantInfo ['scope'] ) ? $this->grantInfo ['scope'] : null, 'username' => $this->grantInfo ['username'], 'password' => $this->grantInfo ['password'] ) );
				$session ['grant_type'] = $this->grantType;
				$this->setSession ( $session );
				return $session ['access_token'];
			}
	...
	protected function oauthTokenRequest(Array $args) {
		...
		if (! isset ( $result )) {
			...
			return array ('access_token' => $result ['access_token'], 'expires' => time () + $result ['expires_in'], 'refresh_token' => isset ( $result ['refresh_token'] ) ? $result ['refresh_token'] : null, 'scope' => isset ( $result ['scope'] ) ? explode ( ' ', $result ['scope'] ) : array () );
		} else {
	...
	protected function httpRequest($url, $payload, $headers = null, &$status_code = null, &$response_headers = null) {
		...
		$headers [] = 'Expect:';
		curl_setopt_array ( $ch, array (CURLOPT_CONNECTTIMEOUT => $this->connectionTimeout, CURLOPT_TIMEOUT => $this->timeout, CURLOPT_PROXY => $this->proxy, CURLOPT_USERAGENT => sprintf ( 'Dailymotion-PHP/%s (PHP %s; %s)', self::VERSION, PHP_VERSION, php_sapi_name () ), CURLOPT_HEADER => true, CURLOPT_RETURNTRANSFER => true, CURLOPT_URL => $url, CURLOPT_HTTPHEADER => $headers, CURLOPT_POSTFIELDS => $payload, CURLOPT_VERBOSE => $this->debug ) );
