...
require_once 'CBUtils/UserAccount.php';
...
class WPTUploadVideoToHost extends CBAbstract {
	public $linearLocation; // array of uploaders that must be run linearly
	...
	function startScheduleUploads() {
		...
		$results = mysql_query ( "Select p.id, p.location from post as p LEFT JOIN uploadsites as us on p.location=us.location where p.posted=0 and us.working=1 and p.attempts<3 group by location, userid order by p.lastattempt desc, rand()" ); // TODO: add where clause to exclude error is null if files aren't being deleted
		...
		if (mysql_num_rows ( $results ) > 0) {
			...
			while ( ($row = mysql_fetch_assoc ( $results )) ) {
				...
				if (in_array ( $location, $this->linearLocation )) {
					...
					//$this->upload($id); // For Test Purposes only
				}
			}
	...
	function upload($id) {
		$query = "Select p.attempts, p.pid, p.location, p.userid , px.port, px.proxy, pc.title, pc.description, k.words From post as p left join products as pc on p.pid=pc.id left join keywords as k on k.id=p.pid left join proxies as px on p.proxyid=px.id Where p.id=$id";
		//echo("Query: $query<br>");
		...
		$userid = $row ["userid"];
		$uAccount = new UserAccount ( $userid, $this->getDBConnection () );
		$location = $row ["location"];
		$userName = $uAccount->getAccountUserName ( $location );
		$password = $uAccount->getAccountPassword ( $location );
		$pid = $row ["pid"];
		$title = $row ["title"];
		...
		*/
		...
		if (isset ( $pid )) {
			...
			$video = new Video ( $pid, $title, $description, $keywords );
			if ($video->isValid ()) {
				switch ($location) {
					case "youtube" :
				...
				if (isset ( $uploader )) {
					$this->getLogger ()->logInfo ( "<br>Uploading Video $pid to $location for User($userid): " . $uAccount->getUserName () );
					$uploader->uploadResponse = $uploader->upload (); // Upload the video
					...
					if ($posted) {
						...
						mysql_query ( $query );
						$this->getLogger ()->logInfo ( "Video Successfully Uploaded!!! Here: <a href='$postURL'>$postURL</a> for User($userid)" );
			...
			$this->getLogger ()->logInfo ( "No Product ID Found for $id" );
