...
class WPTUploadVideoToHost extends CBAbstract {
    ...
    function upload($id) {
        ...
        $query = "update LOW_PRIORITY task set cmd='" . $id . "' where id=" . $this -> taskID;
        ...
        if ($active == true) {
            ...
            if (isset($pid)) {
                ...
                if ($video -> isValid()) {
                    ...
                    if (isset($uploader)) {
                        ...
                        if ($posted) {
                            ...
                            $query = "Update LOW_PRIORITY post SET posted=1, posttime=NOW(), postURL='" . $this -> getDBConnection() -> getDBConnection() -> real_escape_string($postURL) . "' WHERE id=$id";
                            ...
                            $updateQuery = "Update LOW_PRIORITY post SET error='" . $this -> getDBConnection() -> getDBConnection() -> real_escape_string($serverResponse) . "', attempts=$attempts WHERE id=$id";
                ...
                $updateAttempts = "Update LOW_PRIORITY post SET attempts=$attempts WHERE id=$id";
        ...
        echo("<br>"); // Just to add a space between video upload log entries
    }
    ...
    function removeUser($uid, $userName) {
        ...
        $deleteUserQuery = "Update LOW_PRIORITY users set active=false WHERE id=" . $uid;
        ...
        if ($affectedRows > 0) {
            ...
            $inactiveQuery = "Select * from users where active=false AND id=" . $id;
